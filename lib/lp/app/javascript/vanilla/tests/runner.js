...
class VanillaTestRunner {
   ...
   * with the error message; otherwise it passes. The `document.body` is
   * saved before each test and fully restored afterward, so any DOM
   * additions, modifications, or removals are reverted.
   * @param {string} name - Test case name (must be unique within a suite).
   * @param {() => void} fn - Test function to execute.
   */
  test(name, fn) {
    const suite = this.#results[this.#current];
    if (name in suite) {
      throw new Error(
        `Duplicate test name "${name}" in suite "${this.#current}".`
      );
    }
   ...
   *
   * Called automatically when any top-level suite completes.
   */
  done() {
