...
/**
 * Creates a <select> swap trigger with options.
 *
 * opts.swapUrl: swap-url on the select (default "/swap").
 * opts.paramKey: swap-param-key on the select (default "milestone").
 * opts.options: list of { value, paramValue, isDefault, selected, label }.
 * opts.initialHTML: initial content of the target.
 */
function makeSelectFixture(opts) {
  history.replaceState(null, "", location.pathname);
  for (const el of document.querySelectorAll(`[${ATTRIBUTES.isSwapCurrent}]`)) {
    el.removeAttribute(ATTRIBUTES.isSwapCurrent);
  }
  ...
  opts = opts || {};
  const swapUrl = opts.swapUrl || "/swap";
  const paramKey = opts.paramKey || "milestone";
  ...
  select.setAttribute(ATTRIBUTES.swapUrl, swapUrl);
  select.setAttribute(ATTRIBUTES.swapTarget, "#target");
  select.setAttribute(ATTRIBUTES.swapParamKey, paramKey);

  for (const o of opts.options || []) {
    const option = document.createElement("option");
    option.value = o.value;
    option.textContent = o.label || o.value;
    if (o.paramValue !== undefined) {
      option.setAttribute(ATTRIBUTES.swapParamValue, o.paramValue);
    }
    if (o.isDefault) {
      option.setAttribute(ATTRIBUTES.isSwapDefault, "");
    }
    if (o.selected) {
  ...
  return { container, target, select };
}
...
VanillaTest.suite("swap", (T) => {
  ...
  T.suite("SELECTORS.selectElement", (T) => {
    T.test("matches select with swap-url", () => {
      const ctx = makeSelectFixture({
        options: [{ value: "all", isDefault: true }],
      });
      T.ok(ctx.select.matches(SELECTORS.selectElement), "select matches");
    });

    T.test("does not match select without swap-url", () => {
      const el = document.createElement("select");
      T.ok(!el.matches(SELECTORS.selectElement), "plain select does not match");
    });

    T.test("does not match anchor with swap-url", () => {
      const a = document.createElement("a");
      a.setAttribute(ATTRIBUTES.swapUrl, "/x");
      T.ok(!a.matches(SELECTORS.selectElement), "anchor does not match");
    });
  });

  T.suite("getSwapSearchParams (select)", (T) => {
    T.test("uses selected option's swap-param-value", () => {
      const ctx = makeSelectFixture({
        paramKey: "milestone",
        options: [
          { value: "all", isDefault: true },
          { value: "2", paramValue: "2", selected: true, label: "3.1" },
        ],
      });
      const params = getSwapSearchParams(ctx.select);
      T.equal(params.get("milestone"), "2", "uses option's param value");
    });

    T.test("falls back to option.value when swap-param-value missing", () => {
      const ctx = makeSelectFixture({
        paramKey: "milestone",
        options: [
          { value: "all", isDefault: true },
      ...
      const params = getSwapSearchParams(ctx.select);
      T.equal(params.get("milestone"), "7", "falls back to option.value");
    });

    T.test("omits key when selected option has swap-default", () => {
      const ctx = makeSelectFixture({
        paramKey: "milestone",
        options: [
          { value: "all", isDefault: true, selected: true },
          { value: "2", paramValue: "2" },
        ],
      });
      history.replaceState(null, "", location.pathname + "?milestone=5");
      const params = getSwapSearchParams(ctx.select);
      T.ok(!params.has("milestone"), "default option removes key");
    });

    T.test("preserves other URL params", () => {
      const ctx = makeSelectFixture({
        paramKey: "milestone",
        options: [
          { value: "all", isDefault: true },
          { value: "2", paramValue: "2", selected: true },
        ],
      });
      history.replaceState(null, "", location.pathname + "?packages-list=my-uploads");
      const params = getSwapSearchParams(ctx.select);
      T.equal(params.get("milestone"), "2", "select param set");
      T.equal(
        params.get("packages-list"), "my-uploads",
        "other section param preserved"
      );
    });
  });

  T.suite("handleSwapTrigger (select)", (T) => {
    T.test("swaps content using selected option", async () => {
      const ctx = makeSelectFixture({
        swapUrl: "/fetch",
        paramKey: "milestone",
        options: [
      ...
      try {
        await handleSwapTrigger(ctx.select);
        T.ok(
          capturedUrl.includes("milestone=2"),
          "fetch URL has selected option value"
        );
        const newTarget = document.getElementById("target");
        T.equal(newTarget.innerHTML, "<p>new</p>", "content swapped");
      } finally {
    ...
    T.test("clean URL when default option selected", async () => {
      const ctx = makeSelectFixture({
        swapUrl: "/fetch",
        paramKey: "milestone",
        options: [
      ...
      try {
        await handleSwapTrigger(ctx.select);
        T.equal(capturedUrl, "/fetch", "no query string for default option");
      } finally {
        window.fetch = orig;
      }
    });
  ...
  T.suite("change handler", (T) => {
    T.test("fires swap on select change event", async () => {
      const ctx = makeSelectFixture({
        swapUrl: "/fetch",
        paramKey: "milestone",
        options: [
      ...
      try {
        ...
        // Allow microtasks from the change handler to settle.
        await new Promise((r) => setTimeout(r, 0));
        await new Promise((r) => setTimeout(r, 0));
        T.ok(capturedUrl, "fetch was called");
        T.ok(
          capturedUrl.includes("milestone=2"),
          "fetch URL has newly selected value"
        );
      } finally {
        window.fetch = orig;
      }
    ...
    T.test("ignores change on non-swap select", async () => {
      ...
      try {
        ...
        await new Promise((r) => setTimeout(r, 0));
        T.ok(!fetched, "no fetch for plain select");
      } finally {
        window.fetch = orig;
      }
    });
