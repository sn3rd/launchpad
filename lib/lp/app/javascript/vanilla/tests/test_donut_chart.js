...
VanillaTest.suite("donutChart", (T) => {
  ...
  T.suite("drawDonutChart", (T) => {
    ...
    T.suite("gap", (T) => {
      T.test("gap produces different paths than no gap", () => {
        ...
        drawDonutChart(svgGap, segments, defaultOptions({ gap: 0.01 }));
  ...
  // Helper: build a minimal svg + segment path directly wired via
  // initSegmentTooltip so the tooltip logic can be tested in isolation
  // from drawDonutChart's rendering.
  function makeSegment(svg, label) {
    const path = document.createElementNS(svgNS, "path");
    path.classList.add("segment");
    svg.appendChild(path);
    if (label != null) initSegmentTooltip(svg, path, label);
    return path;
  }

  T.suite("initSegmentTooltip", (T) => {
    T.suite("accessibility", (T) => {
      T.test("sets tabindex, role, and aria-label", () => {
        const svg = makeSVG();
        const p = makeSegment(svg, "Slice");
        T.equal(p.getAttribute("tabindex"), "0", "tabindex set");
        T.equal(p.getAttribute("role"), "img", "role set");
        T.equal(p.getAttribute("aria-label"), "Slice", "aria-label set");
      });
    });

    T.suite("mouse", (T) => {
      T.test("mouseenter shows tooltip with label", () => {
        const svg = makeSVG();
        const p = makeSegment(svg, "Red");
        p.dispatchEvent(new MouseEvent("mouseenter", { clientX: 10, clientY: 10, bubbles: true }));
        T.ok(tooltipVisible(), "visible");
        T.equal(tooltipText(), "Red", "label shown");
      });
      ...
      T.test("mouseleave hides tooltip", () => {
        ...
        p.dispatchEvent(new MouseEvent("mouseenter", { clientX: 10, clientY: 10, bubbles: true }));
        p.dispatchEvent(new MouseEvent("mouseleave", { bubbles: true }));
        T.ok(!tooltipVisible(), "hidden");
      });

      T.test("mousemove repositions tooltip", () => {
        const svg = makeSVG();
        const p = makeSegment(svg, "R");
        p.dispatchEvent(new MouseEvent("mouseenter", { clientX: 10, clientY: 20, bubbles: true }));
        const before = tooltipEl().style.left;
        p.dispatchEvent(new MouseEvent("mousemove", { clientX: 100, clientY: 200, bubbles: true }));
        T.ok(before !== tooltipEl().style.left, "moved");
      });
    });
    ...
    T.suite("keyboard", (T) => {
      T.test("focus shows tooltip, blur hides it", () => {
        const svg = makeSVG();
        const p = makeSegment(svg, "Focused");
        p.focus();
        T.ok(tooltipVisible(), "visible on focus");
        T.equal(tooltipText(), "Focused", "label shown");
        p.blur();
        T.ok(!tooltipVisible(), "hidden on blur");
      });
    });

    T.suite("keyboard focus wins over mouse hover", (T) => {
      T.test("mouseenter on focused segment does not reposition tooltip", () => {
        const svg = makeSVG();
        const p = makeSegment(svg, "R");

        p.focus();
        const leftAfterFocus = tooltipEl().style.left;

        p.dispatchEvent(new MouseEvent("mouseenter", { clientX: 999, clientY: 999, bubbles: true }));
        T.equal(tooltipEl().style.left, leftAfterFocus, "position unchanged");
        T.equal(tooltipText(), "R", "label unchanged");
      });

      T.test("mouseenter on another segment ignored while one is focused", () => {
        const svg = makeSVG();
        const a = makeSegment(svg, "A");
        const b = makeSegment(svg, "B");

        a.focus();
        T.equal(tooltipText(), "A", "focus on A");

        b.dispatchEvent(new MouseEvent("mouseenter", { clientX: 500, clientY: 500, bubbles: true }));
        T.equal(tooltipText(), "A", "B's mouseenter ignored");
      });

      T.test("mousemove ignored while a segment is focused", () => {
        const svg = makeSVG();
        const p = makeSegment(svg, "R");

        p.focus();
        const before = tooltipEl().style.left;
        p.dispatchEvent(new MouseEvent("mousemove", { clientX: 999, clientY: 999, bubbles: true }));
        T.equal(tooltipEl().style.left, before, "position unchanged");
      });

      T.test("mouseleave ignored while a segment is focused", () => {
        const svg = makeSVG();
        const p = makeSegment(svg, "R");
        ...
        T.ok(tooltipVisible(), "visible");
        p.dispatchEvent(new MouseEvent("mouseleave", { bubbles: true }));
        T.ok(tooltipVisible(), "still visible");
      });

      T.test("focus is scoped per svg — focus in one doesn't block hover in another", () => {
        const svg1 = makeSVG();
        makeSegment(svg1, "Chart1").focus();
        T.equal(tooltipText(), "Chart1", "chart1 focus tooltip");

        const svg2 = makeSVG();
        const p2 = makeSegment(svg2, "Chart2");
        p2.dispatchEvent(new MouseEvent("mouseenter", { clientX: 10, clientY: 10, bubbles: true }));
        T.equal(tooltipText(), "Chart2", "chart2 hover works — focus scoped per svg");
      });
    });
  });
  ...
  T.suite("initDonutCharts", (T) => {
    ...
    T.test("parses chart-gap and chart-min-ratio", () => {
      ...
      svg.setAttribute("chart-gap", "0.005");
