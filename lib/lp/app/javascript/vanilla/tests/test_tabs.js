 ...
 * @param {string} key
 * @param {{ altKey?: boolean, ctrlKey?: boolean, metaKey?: boolean }} [mods]
 */
function pressKey(element, key, mods) {
...
VanillaTest.suite("tabs", (T) => {
  T.suite("handleKeydown", (T) => {
    ...
    T.test("Alt+ArrowRight does not move focus (browser back/forward)", () => {
      const ctx = makeTablist(3);
      initTabsWithin(ctx.container);

      ctx.tabs[0].focus();
      pressKey(ctx.tabs[0], "ArrowRight", { altKey: true });
      T.equal(document.activeElement, ctx.tabs[0], "focus unchanged");
    });

    T.test("Alt+ArrowLeft does not move focus (browser back/forward)", () => {
      const ctx = makeTablist(3);
      initTabsWithin(ctx.container);

      ctx.tabs[1].focus();
      pressKey(ctx.tabs[1], "ArrowLeft", { altKey: true });
      T.equal(document.activeElement, ctx.tabs[1], "focus unchanged");
    });

    T.test("Ctrl+ArrowRight does not move focus", () => {
      const ctx = makeTablist(3);
      initTabsWithin(ctx.container);

      ctx.tabs[0].focus();
      pressKey(ctx.tabs[0], "ArrowRight", { ctrlKey: true });
      T.equal(document.activeElement, ctx.tabs[0], "focus unchanged");
    });

    T.test("Meta+ArrowRight does not move focus", () => {
      const ctx = makeTablist(3);
      initTabsWithin(ctx.container);

      ctx.tabs[0].focus();
      pressKey(ctx.tabs[0], "ArrowRight", { metaKey: true });
      T.equal(document.activeElement, ctx.tabs[0], "focus unchanged");
    });
  ...
  T.suite("initTabsWithin (hydration)", (T) => {
    ...
    T.test("restoreFocus focuses the selected tab", () => {
      const ctx = makeTablist(3, { activeIndex: 1 });

      initTabsWithin(ctx.container, true);

      T.equal(document.activeElement, ctx.tabs[1], "selected tab is focused");
    });

    T.test("restoreFocus does not focus without flag", () => {
      const ctx = makeTablist(3, { activeIndex: 1 });
      document.body.focus();
      ...
      T.ok(
        document.activeElement !== ctx.tabs[1],
        "selected tab is not focused"
      );
    });
  });
});
