 ...
 *
 * Anchor triggers fire on click. Select triggers (swap-url on the
 * `<select>`) fire on change; the selected `<option>` provides
 * swap-param-value (falling back to its `value`) and swap-default.
 *
...
export const SELECTORS = {
  ...
  selectElement: `select[${ATTRIBUTES.swapUrl}]`,
};
...
export function getSwapSearchParams(trigger) {
  ...
  const isSelect = trigger.tagName === "SELECT";
  const source = isSelect
    ? (trigger.selectedOptions[0] ?? trigger)
    : trigger;
  const triggerParamKey = trigger.getAttribute(ATTRIBUTES.swapParamKey);
  let triggerParamValue = source.getAttribute(ATTRIBUTES.swapParamValue);
  if (isSelect && triggerParamValue === null) {
    triggerParamValue = source.value;
  }
  const triggerIsDefaultValue = source.hasAttribute(ATTRIBUTES.isSwapDefault);
...
document.addEventListener("change", async (e) => {
  const trigger = e.target.closest(SELECTORS.selectElement);
  if (!trigger) return;
  try {
    await handleSwapTrigger(trigger);
  } catch (err) {
