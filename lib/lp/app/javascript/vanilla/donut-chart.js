 ...
 *
 * HTML attributes:
 *   chart-segments   – JSON array of { value: number, fill: string }
...
export function drawDonutChart(svg, segments, options) {
  ...
  for (const a of arcs) {
    ...
    path.setAttribute("fill", a.data.fill)
...
export function initDonutCharts(root) {
  ...
  for (const donutChart of donutCharts) {
    ...
    const gap = parseAttrInt(donutChart.getAttribute(ATTRIBUTES.chartGap), 0)
