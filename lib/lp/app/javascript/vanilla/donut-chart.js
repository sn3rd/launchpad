...
function hasFocusedSegment(svg) {
  return svg.querySelector(".segment:focus") !== null
}

/**
 * Wires hover + keyboard focus tooltip behavior for a labelled segment.
 * Keyboard focus wins: while any segment in `svg` is focused, mouse hover
 * events on segments are ignored. On blur, the tooltip is handed back to
 * whichever segment the mouse is currently over.
 *
 * @param {SVGSVGElement} svg
 * @param {SVGPathElement} path
 * @param {string} label
 */
export function initSegmentTooltip(svg, path, label) {
  path.setAttribute("tabindex", "0")
  path.setAttribute("role", "img")
  path.setAttribute("aria-label", label)

  path.addEventListener("mouseenter", (e) => {
    if (hasFocusedSegment(svg)) return
  ...
  path.addEventListener("focus", () => {
    ...
    showTooltip(label, rect.left + rect.width / 2, rect.top + rect.height / 2)
  })
  path.addEventListener("blur", () => {
    // If the mouse is over a segment, hand the tooltip back to it.
    const hovered = svg.querySelector(".segment:hover")
...
export function drawDonutChart(svg, segments, options) {
  ...
  d3.select(svg)
    ...
    .each(function (a) {
      ...
      if (a.data.label) initSegmentTooltip(svg, this, a.data.label)
    })
